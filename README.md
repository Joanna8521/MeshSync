...
<!-- INSTALL:START -->
拿到 `mesh-sync-local-v*.zip` 之後，三分鐘就能裝好。

### 步驟

1. **解壓縮**，會看到一個 `mesh-sync-extension` 資料夾。
   把它放到你放得住的地方（例如「文件」）。
...
3. 按左上角**「載入未封裝項目」**，選取那個資料夾
   （選到裡面看得到 `manifest.json` 的那一層），按「選取」。

4. 裝好後會自動打開設定導覽頁，照著做兩件事：
   - 點工具列的 Mesh Sync 圖示 → **「連接 Google」** →
     用自己的 Google 帳號授權。你的內容只會寫進**你自己的**雲端硬碟。
   - 把導覽頁上那段 **prompt 複製**，貼到 ChatGPT 自訂指令
...
- **按了沒反應**
  回到對話分頁按 `Cmd+R`（Windows 是 `F5`）重新整理。
  擴充需要頁面重新載入才會生效。

- **想確認有沒有正常運作**
  點擴充圖示 → 按「檢查」，會顯示這個分頁偵測到幾則訊息。

- **Chrome 重開後跳出「停用開發人員模式擴充功能」**
  按「取消」就好，這是 Chrome 對未上架擴充的例行提醒。

- **「連接 Google」出現錯誤**
  多半是授權視窗被關掉或逾時，再按一次即可。
<!-- INSTALL:END -->
